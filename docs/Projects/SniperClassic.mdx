...
But when translating it 1:1 to 3D, it becomes apparent in-game that it feels very different from how the character plays in 2D.

With this in mind, I played many runs with Sniper in the original Risk of Rain to figure out the character, and came up with these core takeaways about the character's identity:
...
When adapting Sniper to 3D, the first thing that becomes immediately apparent is that a piercing shot does not have the same crowd control potential that it has in 2D.
...
When adapting Sniper into Risk of Rain 2, I wanted to add more to this skill, so that it would feel like a full-fledged attack with its own learning curve for using it.
...
Then you shoot the enemy, and a portion of the damage is transferred to any other enemy near them!
